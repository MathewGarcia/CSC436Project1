...
//import './Convertcss.css'; //for debugging we remove this
import axios from 'axios';
import PropTypes from 'prop-types';

function Convert({ inputCurrency, selected }) {
	Convert.propTypes = {
	...
	const fetchAPIData = async () => {
		...
		if (
			...
			Date.now() - localStorage.getItem('fetchTime') > FIVE_MIN
		) {
	...
	const fetchAPITime = async () => {
		...
		if (
			(localStorage.getItem('fetchTime') &&
				Date.now() - localStorage.getItem('fetchTime') >= FIVE_MIN) ||
			localStorage.getItem('localTime') === null ||
			localStorage.getItem('localTime').length === 0
		) {
		...
		if (selected === 'usd') {
			if (usdButtonName === 'USD-BTC') {
				...
				setConversionCurrency(
					conversionCurrency * parseFloat(bpiData.USD.rate_float)
				);
	...
	const eurHandler = () => {
		if (selected === 'eur') {
			if (eurButtonName === 'EUR-BTC') {
				setError('');
				setConversionCurrency(
					inputCurrency / parseFloat(bpiData.EUR.rate_float, 10.0)
				);
				setEURButtonName('BTC-EUR');
			} else {
				setConversionCurrency(
					conversionCurrency * parseFloat(bpiData.EUR.rate_float)
				);
	...
	const gbpHandler = () => {
		if (selected === 'gbp') {
			if (gbpButtonName === 'GBP-BTC') {
				setError('');
				setConversionCurrency(
					inputCurrency / parseFloat(bpiData.GBP.rate_float, 10.0)
				);
				setGBPButtonName('BTC-GBP');
			} else {
				setConversionCurrency(
					conversionCurrency * parseFloat(bpiData.GBP.rate_float)
				);
			...
			{!loading && (
				<>
					<div className="button-container">
						<img
							style={{ height: 50 }}
							src="https://media.tenor.com/Mfvl4UTuR2kAAAAS/bitcoin.gif"
							alt=""
						/>
						<button type="btn" className="bg-black" onClick={usdHandler}>
							{usdButtonName}{' '}
						</button>
						<button type="btn" onClick={eurHandler}>
							{' '}
							{eurButtonName}{' '}
						</button>
						<button type="btn" onClick={gbpHandler}>
							{' '}
							{gbpButtonName}{' '}
						</button>
						<button type="btn" onClick={refreshHandler}>
							Refetch
						</button>
					</div>
					<h2>
						{' '}
						{selected.toUpperCase()} {bpiData[selected.toUpperCase()]?.rate_float}
					</h2>
					...
					<p style={{ color: 'white' }}>
						Last fetched time: {localStorage.getItem('localTime')}
					</p>
				</>
			)}
		</div>
